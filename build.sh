...
# Usage: ./build.sh [mango|kde|kinectic|kde-nvidia-open] [tag]
...
# CONTAINERFILE selects which Dockerfile the build uses. kde-nvidia-open
# needs the two extra akmods FROM stages that only live in
...
case "$DESKTOP" in
  ...
  kde-nvidia-open)
    ...
    ;;
  *)
    echo "Unknown desktop: $DESKTOP (expected: mango | kde | kinectic | kde-nvidia-open)" >&2
    exit 1
    ;;
esac
